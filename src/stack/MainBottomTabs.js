...
import { TouchableOpacity, View, Platform, StyleSheet, I18nManager, SafeAreaView } from 'react-native';
...
const MyTabBar = ({ state, descriptors, navigation, screen, setScreen }) => {
  ...
  return state.routes[0].state?.index === 1 ? null : (
    <View style={[styles.tabBottomBar, { backgroundColor: screen === 'ConversationScreen' ? colors.white : colors.dark, height: 50, marginBottom: insets.bottom }]}>
  ...
  return (
    ...
    <MainStackContextProvider>
      <Tab.Navigator
        ...
        tabBar={props => <MyTabBar {...props} screen={screen} setScreen={setScreen} />}
